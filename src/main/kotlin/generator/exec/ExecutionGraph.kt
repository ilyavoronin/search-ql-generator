...
class ExecutionGraph(val scheme: GeneratorScheme, val genObjects: GeneratedObjects, val findQuery: FindQuery) {
    ...
    fun execute(order: ExecutionOrder): List<GeneratedObject> {
        ...
        val finalRes = (executionResult[root.id] as PathExecutionResult)
        val result = mutableSetOf<GeneratedObject>()

        finalRes.objRetriever?.let { result.addAll(it.retrieve(object : GeneratedObject {
            override fun hashCode(): Int = 0
            override fun equals(other: Any?) = false
        }, null)) }
        finalRes.bottomLevelObjects?.let { result.addAll(it) }
            ...
            return when {
                        ...
                        ExecType.SourceObjCalc -> {
                            ...
                            if (contextParent.parent != null) {
                                ...
                                val ret = ObjRetriever { _, _ ->
                                    ...
                                    filterObjectsBottomUp(filteredObjects, rootPath!!)
                                }

                                val resObjs = res.objs?.let { filterObjectsBottomUp(it, rootPath!!) }
                                PathExecutionResult(ret, resObjs, CombinedObjFilter(ObjRevFilterBuilder { _ ->
                                    ObjFilter { res.objFilter?.accepts(it) ?: true }
                                }, null))
                            }
                        }
                ...
                cond == null && subPath != null -> {
                    ...
                    when (t) {
                        ...
                        ExecType.SourceObjCalc -> {
                            if (contextParent.parent != null || resCond.objFilter == null || resPath.combinedObjFilter.objRetriever != null) {
                                val finalObjs = if (resCond.objFilter == null) {
                                    resCond.objs!!
                                } else if (resPath.objRetriever != null) {
                                    val objs = genObjects.getAllObjectsOfType(obj.name)
                                ...
                                resPath.objRetriever?.let { ret ->
                                    finalSobjs.addAll(filteredFinalObjects.flatMap {
                                        ret.retrieve(
                                            it,
                                            null
                                        )
                                ...
                                // top level objects case, in order to not calculate all objects when there is PathAndExecNode above
                                val retriever = ObjRetriever {_, _ ->
                                    val result = if (resPath.objRetriever != null) {
                                        val objs = genObjects.getAllObjectsOfType(obj.name)
                                        val final =
                                            objs.filter { resCond.objFilter.accepts(it) || resCond.objs?.contains(it) ?: false }
                                        val filteredFinalObjects = filterObjectsBottomUp(final, rootPath!!)
                                        resPath.objRetriever.let { ret ->
                                            filteredFinalObjects.flatMap {
                                                ret.retrieve(
                                                    it,
                                                    null
                                                )
                                ...
                                PathExecutionResult(retriever, resPath.bottomLevelObjects, CombinedObjFilter(objFilterBuilder, null))
                            }
                        }
                        ExecType.SourcePropertyCalc -> {
    ...
    inner class ObjAndExecNode(l: ObjCondExecutionNode, r: ObjCondExecutionNode) : ObjCondExecutionNode() {
        ...
        override fun executeInternal(t: ExecType): ExecutionResult {
            ...
            val right = getChildResult(1) as ObjExecutionResult
            ...
            if (left.objs != null && right.objs != null) {
                resObjs?.addAll(left.objs.intersect(right.objs))
            }
        ...
        override fun executeInternal(t: ExecType): ExecutionResult {
            ...
            return when (t) {
                ...
                ExecType.SourceObjCalc -> {
                    ...
                    val revMethod = genObjects.getRevDefMethod(obj.name, extendedDefField.parent!!.name)
                    ObjExecutionResult(null, resObjs.flatMap { revMethod.call(it, extendedDefField, modifiers)}.toSet())
                }
                ExecType.SourcePropertyCalc -> {
    ...
    fun buildPathToSobj(
        ...
        for ((i, subObj) in currObj.members.withIndex()) {
            ...
            newPathToRoot?.add(
                RootPathElem(
                    -1,
                    genObjects.getRevDefMethod(subObj.memType, currObj.name),
                    parentObjField,
                    mapOf()
                )
            )
